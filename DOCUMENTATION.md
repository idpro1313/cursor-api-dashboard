...
- **Настройки:** ввод API key, сохранение в БД (кнопка «Сохранить ключ в БД»). Если ключ уже сохранён, отображается «Ключ сохранён в БД» и ссылка «Изменить ключ». При выгрузке без ключа или с недействительным ключом форма ввода показывается с подсказкой.
- **Сохранение в локальную БД:**
  ...
  - Кнопка «Загрузить и сохранить в БД» запускает потоковую синхронизацию (`/api/sync-stream`): отображаются прогресс-бар и текст шага (эндпоинт, период, число записей). Запрашиваются только недостающие диапазоны дат: для каждого эндпоинта вычисляются дни, которых ещё нет в БД, и формируются чанки по 30 дней только по этим диапазонам (одни и те же данные не запрашиваются повторно).
