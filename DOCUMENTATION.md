...
- **Данные в БД:** покрытие БД, фильтры по эндпоинту и датам. Кнопка «Показать данные» — карточки по датам и эндпоинтам. Для Usage Events поля `tokenUsage` развёрнуты в отдельные колонки.
- **Jira:** текущий список пользователей из БД (таблица), кнопки «Обновить» и «Очистить данные Jira».
- **Счета:** список загруженных счетов, просмотр позиций счёта (description, qty, unit price, tax, amount, date of issue), удаление счёта; единая таблица «Все позиции по счетам».
