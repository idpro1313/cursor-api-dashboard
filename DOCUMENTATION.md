...
- **Сводка:** запросы, строки ±, события и стоимость Usage Events, токены (М/К), траты Spend API, участники команды, стоимость по моделям (таблица).
...
| **Spending Data** | Дашборд (сводка и по пользователям) | Траты из `/teams/spend` в сводке и в карточках/таблице пользователей. |
