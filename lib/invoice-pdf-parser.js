...
function extractTableFromRows(rows) {
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    const hasNumbers = amountCents != null || (qtyVal != null && qtyVal >= 0 && qtyVal < 10000) || (taxPct != null && taxPct >= 0 && taxPct <= 100);

    if (hasNumbers) {
      const description = pendingDescription.length ? pendingDescription.join(' ').trim() : (desc || null);
      const cleanDesc = description ? description.replace(/\s+/g, ' ').replace(/^[\s—]+|[\s—]+$/g, '').trim() || null;
      out.push({
        row_index: rowIndex++,
        description: cleanDesc || null,
        quantity: qtyVal != null && qtyVal >= 0 && qtyVal < 10000 ? qtyVal : null,
      ...
      if (desc && !/^[\s—]+$/.test(desc)) pendingDescription.push(desc);
    }
  }
