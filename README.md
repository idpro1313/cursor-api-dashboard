...
- **Структура (по логину и паролю):**
  ...
  - **Просмотр данных:** Дашборд, Участники и расходы, **Данные** (вкладки: Данные в БД с выгрузкой JSON по эндпоинтам, Jira, Счета с типами начислений и выгрузкой JSON, кнопка «Очистить БД от счетов», Аудит), **Отчёт по счетам** (сводка по периодам биллинга и типам начислений), **Сверка** (Usage Events и счета по периодам).
