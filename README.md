...
Страница **«Дашборд по пользователям»** (`users-dashboard.html`) показывает список пользователей из Jira и активность использования Cursor по неделям. Источники данных: таблица **Пользователи Jira** (CSV) и эндпоинт **Daily Usage Data** в БД. Сопоставление пользователей выполняется по email (поля «Внешний почтовый адрес», «Email» и т.п. в CSV). В таблице можно выбрать метрику: дни с активностью, число запросов (composer/chat/agent), строки кода (+/−). Перед использованием загрузите пользователей Jira (CSV) и выполните синхронизацию Daily Usage в БД.

- **API:** `GET /api/users/activity-by-week?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD` — пользователи Jira и агрегированная активность по неделям.
