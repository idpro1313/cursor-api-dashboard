...
- **Главная страница (дашборд)** — без входа: активность по пользователям и месяцам (запросы, строки кода, события и стоимость Usage Events, токены, траты Spend API), сводка по моделям, блок «Активные в Jira, но не используют Cursor», затраты по проекту помесячно. Источники: Daily Usage Data, Usage Events, Jira (CSV), Team Members, Spending Data.
